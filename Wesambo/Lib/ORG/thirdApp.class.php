...
class thirdApp {
	...
	public function __construct(){
		$this->serverUrl='http://up.weike.me/';
	...
	public function modules(){
		...
		return array('music','yinle','kuaidi');
	}
	public function yinle($name){
		return $this->music($name);
	}
	public function music($name){ //虾米音乐API
		$name = implode('',$name);
		$url= 'http://'. $_SERVER['SERVER_NAME'] .'/api/xiami.php?key=' . $name;
		$rt = $this->curlGet($url);
		//$rt = file_get_contents($url);
		$array = (Array)json_decode($rt);
		if($array != null){
			return array(array($array[0]->title,$array[0]->author,$array[0]->src,$array[0]->src),'music');
		}else{
