var SocketIp = "kf.wesambocms.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.wesambocms.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.Wesamboid && obj.Wesamboid.length > 0) {
                    t.initialize(options, obj.Wesamboid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"Wesamboid":"' + obj.Wesamboid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, Wesamboid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&Wesamboid=' + Wesamboid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, Wesamboid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, Wesamboid);
    },
    GetPolling: function (AId, Wesamboid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&Wesamboid=' + Wesamboid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.Wesamboid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.Wesamboid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.Wesamboid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, Wesamboid, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, Wesamboid); });
        }
    ...
    socketconnect: function (options, Wesamboid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, Wesamboid);
                        }
                    }
                });
            });
