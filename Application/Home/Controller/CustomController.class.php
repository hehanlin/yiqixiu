...
class CustomController extends Controller{
	...
	public function exp(){
		...
		$data = ',,,,,,,,,,,,'.utf2gb('技术支持 By 一秀').','."\n";  		
		$data .= ',total:'.$_scene_count.',,,,,,,,,,,,'."\n"; 
 		$data .=  utf2gb('姓名').','.utf2gb('性别').','.utf2gb('手机').','.utf2gb('邮箱').','.utf2gb('公司').','
			.utf2gb('职务').','.utf2gb('固定电话').','.utf2gb('地址').','.utf2gb('网址').','.utf2gb('QQ号').','.i('微信号').','.utf2gb('易信号').','.utf2gb('来往号').','.utf2gb('其它').''."\n";  
		
		foreach($_scene_list as $m_info){
			
			$data.=utf2gb($m_info['name']).','. utf2gb($m_info['sex']).','. utf2gb($m_info['mobile'] ).','. i($m_info['email'] ).','. utf2gb($m_info['company'] ).','
