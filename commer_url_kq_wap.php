...
if ($_GET) {
	$arr = $_GET;
/*	if (0){
		v $get_para = 'zhuanru_money=' . $total_fee;
		v $get_para .= '&uid=' . session('uid');
		v $get_para .= '&kuaiqian_account=' . $kuaiqian_account;
		v $get_para .= '&kuaiqian_key=' . $kuaiqian_key;
		$get_para .= '&out_trade_no=' . $out_trade_no;	//订单号
	}*/
	$user_id = $arr['uid'];
	...
	$success_ret_url = base64_decode($arr['return_url']); //成功后的返回地址
} else {
	exit('forbidden');
}
